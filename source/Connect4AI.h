// Header file for functions that cann be accessed outside of the C file.
#pragma once

void initialiseGame(void);		// Initialise the game including reading the weightings from file.
...
int calculateMove(void);		// Call to calculate the computer move which is added to the game table.
...
void newWeightings(void);		// Function to go through previous moves to adjust weightings.

void changeDifficulty(void);	// Function to move the difficulty on one.

int getDifficulty(void);		// Accessor for difficulty.
